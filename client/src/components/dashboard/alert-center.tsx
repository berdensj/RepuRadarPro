...
export function AlertCenter() {
  ...
  // Sample keyword trends data for display, used as a fallback if fetched data is empty or during loading.
    ...
    <Card className="h-full">
      ...
      <CardContent className="space-y-5">
        ...
        <div>
          ...
          {/* // TODO: Add UI error state display if API calls fail. */}
          {isLoadingAlerts ? (
            <div className="space-y-2">
              {[1, 2, 3].map(i => (
          ...
          ) : keywordTrends && keywordTrends.length > 0 ? ( // FIXED: Use fetched keywordTrends
            <div className="space-y-2">
              {keywordTrends.map((trend: any, index: number) => ( //FIXED: Added type any for trend, consider defining a specific type
                <div 
                  key={index} 
                  className={`p-3 rounded-md border-l-4 ${
                    // TODO: Determine a more robust way to style positive/negative trends
                    trend.keyword === "Staff Friendliness" || (trend.trend && trend.trend.includes('positive'))
                ...
                >
                  <div className="flex justify-between">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      ...
                      {/* TODO: Ensure trend.trend is consistently 'increasing' or 'decreasing' for +/- prefix */}
